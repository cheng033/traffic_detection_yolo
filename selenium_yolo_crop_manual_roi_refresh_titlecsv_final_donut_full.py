...
while True:
    ...
    if boxes is not None:
        ...
        for box, tid, cls, conf in zip(boxes.xyxy.cpu().numpy(), ids, classes, confs):
            ...
            cv2.putText(crop, label, (x1, y1 - 8),
            ...
            for i in range(NUM_AREAS):
                ...
                if tid not in counted[i] and is_overlap(det_box, area_box):
                    ...
                    log_records.append([i + 1, tid, cname, datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
...
with open(csv_filename, "w", newline="", encoding='utf-8-sig') as f:
    writer = csv.writer(f)
    writer.writerow(["Area", "TrackID", "Class", "Timestamp"])
